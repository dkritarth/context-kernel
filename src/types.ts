 ...
 * in wrangler.toml(.example): two KV namespaces (one for curated context +
 * journal, one for OAuth state) plus the two auth secrets. This is also
 * structurally compatible with `AuthEnv` from `src/mcp/auth.ts` (which only
 * needs the two token fields), so it can be passed anywhere an `AuthEnv` is
 * expected.
 */
export interface Env {
  /** Curated context (read path) + journal buffer (write path). */
